...
onAuthStateChanged(auth, (user) => {
    if (user) {
        ...
        // Only add this event listener ONCE, inside this block:
        document.getElementById('profile-img').addEventListener('click', () => {
            uploadcare.openDialog(null, { publicKey: UPLOADCARE_PUBLIC_KEY, imagesOnly: true, crop: '1:1' })
                .done(function(file) {
                    file.done(async function(info) {
                        const photoURL = info.cdnUrl;
                        try {
                            // Update Firebase Auth profile so it persists
...
// To trigger the widget when clicking the image:
document.getElementById('profile-img').addEventListener('click', () => {
    uploadcare.openDialog(null, { publicKey: UPLOADCARE_PUBLIC_KEY }).done(function(file) {
